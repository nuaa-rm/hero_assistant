...
/**
 * @brief 将custom msg格式的点云数据存入pl_surf中，再赋给pcl_out
 * 
 * @param msg 
 * @param pcl_out 
 */
...
/**
 * @brief 将有效的,根据point_filter_num下采样(这个是下采样吗，感觉有点奇怪)的，符合blind条件的，体素滤波过的点加入pl_surf中；
 *        并根据CustomPoint的offset_time，给出每个点的精确时间点curvature，表示每个点相对这一帧第一个点的时间戳；
 * 
 * @param msg 
 */
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  for(uint i=1; i<plsize; i++)
  {
    ...
    {
      valid_num ++;
      if (valid_num % point_filter_num == 0) //根据point_filter_num下采样有效点
      {
        ...
        if((abs(pl_full[i].x - pl_full[i-1].x) > 1e-7 || abs(pl_full[i].y - pl_full[i-1].y) > 1e-7|| abs(pl_full[i].z - pl_full[i-1].z) > 1e-7)
              ...
              //   (pl_full[i].y > -0.646 * pl_full[i].x + 0.3
              // || pl_full[i].y < 1.548 * pl_full[i].x - 0.72 
              // || pl_full[i].y < -0.646 * pl_full[i].x - 0.7
              // || pl_full[i].y > 1.548 * pl_full[i].x + 0.7 ))
