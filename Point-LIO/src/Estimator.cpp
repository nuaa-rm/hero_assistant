...
PointCloudXYZI::Ptr normvec(new PointCloudXYZI(100000, 1));//点的法向量
std::vector<int> time_seq;
///feats_down_body：进行下采样时将feats_down_body中点云下采样，不下采样则直接获取Measures.lidar NO.2
PointCloudXYZI::Ptr feats_down_body(new PointCloudXYZI());
///feats_down_world: 由feats_down_body转坐标系得到的世界坐标系点云 NO.2
PointCloudXYZI::Ptr feats_down_world(new PointCloudXYZI());
PointCloudXYZI::Ptr feats_down_obstacle(new PointCloudXYZI()); //进行过下采样的障碍物点云
...
void h_model_output(state_output &s, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		p_world << point_world_j.x, point_world_j.y, point_world_j.z;
		{
			...
			{
				point_selected_surf[idx+j+1] = false;
				if (esti_plane(pabcd, points_near, plane_thr)) //(planeValid) //判断邻近点是否能组成一个平面
				{
					//计算point_world_j到拟合平面的距离
					float pd2 = pabcd(0) * point_world_j.x + pabcd(1) * point_world_j.y + pabcd(2) * point_world_j.z + pabcd(3);
					...
					if (p_body.norm() > match_s * pd2 * pd2)
					{
						...
						normvec->points[j].intensity = pabcd(3);
						effect_num_k ++;
						float gradient = acos(sqrt(pow(normvec->points[j].x, 2) + 
										pow(normvec->points[j].y, 2)) / sqrt(pow(normvec->points[j].x, 2) + 
										pow(normvec->points[j].y, 2) + 
										pow(normvec->points[j].z, 2)));
						// 如果法向量与地面的夹角大于角度阈值
						if (abs(gradient) < angle_threshold_)
						{
							feats_down_obstacle->points.push_back(feats_down_body->points[idx+j+1]);
						}
					}
				}  
			}
...
/**
 * @brief 将雷达坐标系下的点根据外参Lidar_R/T_wrt_IMU转换到imu坐标系，再转换到世界坐标系下
 *
 * @param pi 转换前livox坐标系（？）下的点
 * @param po 转换后世界坐标系下的点
 */
