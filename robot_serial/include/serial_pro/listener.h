...
namespace ms {
    ...
    private:
        ...
        bool scan() {
            ...
            // for (int i = 0; i < rxBuffer.size() - 6; i++) {
            //     if((uint8_t)rxBuffer[i] == 0xA5 && /*(uint8_t)rxBuffer[i+1] == 0x0B && (uint8_t)rxBuffer[i+2] == 0x00 &&*/ (uint8_t)rxBuffer[i+5] == 0x03 && (uint8_t)rxBuffer[i+6]==0x03){
            //         std::cout<<"yes"<<(int)rxBuffer[i+1]<<" "<<(int)rxBuffer[i+2]<<std::endl;
            //     }
            // }
