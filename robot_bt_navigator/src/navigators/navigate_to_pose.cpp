...
#include <memory>
#include <limits>
#include "robot_bt_navigator/navigators/navigate_to_pose.hpp"
  ...
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_to_pose_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("robot_bt_navigator");
    node->declare_parameter<std::string>(
      "default_nav_to_pose_bt_xml",
      pkg_share_dir +
      "/behavior_trees/navigate_to_pose_w_replanning_and_recovery.xml");
  }
  // 从这段代码可以看出，default_nav_to_pose_bt_xml这个参数需要给完整路径，而不是单纯文件名
...
NavigateToPoseNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  RCLCPP_INFO(logger_, "Receive goal, using %s", bt_xml_filename.c_str());
...
{
  RCLCPP_INFO(logger_, "navigate to pose completed");
}
...
NavigateToPoseNavigator::onLoop()
{
  ...
  try {
    ...
    // RCLCPP_INFO(logger_, "navigate to pose onloop, distance remaining: %lf", distance_remaining);
